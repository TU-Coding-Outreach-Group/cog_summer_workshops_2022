...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      # Panel for Fear_During
      selectInput(inputId = "Fear_During",
                  label = "Fear Rating During Haunted House:",
                  choices = c("1", "2", "3", "4", "5", "All"),
                  selected = "All",
                  multiple = F),
      
      # Panel for Strategy
      selectInput(inputId = "Strategy",
                  label = "Regulation Strategy:",
                  choices = c("Reappraisal", "Distraction", "Both"),
                  selected = c("Both"),
                  multiple = F),
...
server <- function(input, output) {
  ...
  output$regressPlot <- renderPlot({
    ...
    # Subsetting strategy if we don't want both
    if (input$Strategy != "Both"){
      df_temp <- subset(df_temp, df_temp$Strategy %in% input$Strategy)
    }
    ...
    # Subsetting Fear if we don't want all
    if (input$Fear_During != "All"){
      df_temp <- subset(df_temp, df_temp$Fear_During %in% as.numeric(input$Fear_During))
    }
