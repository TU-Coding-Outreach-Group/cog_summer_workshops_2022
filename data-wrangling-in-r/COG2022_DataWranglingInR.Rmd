...
author: 
  ...
  - name: "Content editor: **Billy Mitchell**"
output:
  html_document:
...
```
NOTE: If you receive an error stating: ```error: JAVA_HOME cannot be determined from the Registry```, try [this troubleshooting page to resolve the issue](https://www.r-statistics.com/2012/08/how-to-load-the-rjava-package-after-the-error-java_home-cannot-be-determined-from-the-registry/). [This source](https://statisticsglobe.com/r-error-java-home-cannot-be-determined-from-the-registry#:~:text=So%20why%20does%20the%20error,64%2Dbit%20version%20of%20Java.) also suggests that it may be because you are using a 32-bit version of Java and a 64-bit version of R. Downloading a 64-bit version of both should fix the issue.
...
#change the directory to where you have the COG2022_DataWranglinginR.csv saved
wd <- "C:/Users/Administrator/Documents/GitHub/cog_summer_workshops_2022/data-wrangling-in-r/"
setwd(wd)
...
```{r Copying the dataframe to a new object}
...
```{r Checking Structure of dataframe}
...
```{r Checking column names of dataframe}
...
```{r A meaningless calculation but less legible}
set.seed(0)
...
```{r A meaningless calculation but now cooler}
set.seed(0)
...
```{r Cleaning Useless Pipe Text Example Values, eval=TRUE, include=FALSE}
rm(final, x, y, zr)
```
